...
/**
 * @title AnonAadhaarRecoveryModule
 ...
 * recovery request is
 */
contract AnonAadhaarRecoveryModule is
    ERC7579ExecutorBase,
    IAnonAadhaarRecoveryModule
{
    /*´:°•.°+.*•´.*:˚.°*.˚•´.°:°•.°•.*•´.*:˚.°*.˚•´.°:°•.°+.*•´.*:*/
    /*                    CONSTANTS & STORAGE                     */
    /*.•°:°.´+˚.*°.˚:*.´•*.+°.•°:´*.´•*.•°.•°:°.´:•˚°.*°.˚:*.´+°.•*/
     ...
     */
    address public immutable anonAadhaarRecoveryManager;
    ...
    constructor(
        address _anonAadhaarRecoveryManager,
        address _validator,
        bytes4 _selector
    ) {
        if (
        ...
        anonAadhaarRecoveryManager = _anonAadhaarRecoveryManager;
    ...
    function onInstall(bytes calldata data) external {
        ...
        (
            ...
            uint256 expiry
                ...
                data,
                (bytes, uint[], uint256[], uint256, uint256, uint256)
            );
        ...
        _execute({
            to: anonAadhaarRecoveryManager,
            value: 0,
            data: abi.encodeCall(
                IAnonAadhaarRecoveryManager.configureRecovery,
                (guardians, weights, threshold, delay, expiry)
            )
        });
    }
    ...
    function onUninstall(bytes calldata /* data */) external {
        ...
        IAnonAadhaarRecoveryManager(anonAadhaarRecoveryManager)
            .deInitRecoveryFromModule(msg.sender);
    }
    ...
    function recover(
        ...
        if (msg.sender != anonAadhaarRecoveryManager) {
    ...
    function getTrustedRecoveryManager() external view returns (address) {
        return anonAadhaarRecoveryManager;
    }
    ...
    function name() external pure returns (string memory) {
        return "AnonAadhaar.AnonAadhaarRecoveryModule";
    }
